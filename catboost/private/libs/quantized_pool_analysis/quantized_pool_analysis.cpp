...
namespace NCB {
    ...
    namespace {
        ...
        public:
            explicit TFloatFeatureHolderGenerator(size_t featureNum, const TVector<float>& featureValues)
                : FeatureNum(featureNum)
                , FeatureValues(featureValues)
                , PrevBorder(FeatureValues[0])
                , CurrentIndex(0) {
            }

            template <class TTSubsetIndexing>
            THolder<TFloatValuesHolder> Next(size_t objectCount, const TTSubsetIndexing* subsetIndexing) {
                ...
                } else if (CurrentIndex == FeatureValues.size()) {
                    value = std::numeric_limits<float>::max();
                } else {
                    float newBorder = FeatureValues[CurrentIndex];
                    value = (PrevBorder + newBorder) / 2.f;
        ...
        public:
            ...
            Next(size_t objectCount, const TTSubsetIndexing* subsetIndexing) {
        ...
        public:
            explicit TCatFeatureHolderGenerator(
                ...
                const TVector<size_t>& ignoredValues)
            ...
            template <class TTSubsetIndexing>
            THolder<THashedCatValuesHolder> Next(size_t objectCount, const TTSubsetIndexing* subsetIndexing) {
                while (IgnoreIndex < IgnoredValues.size() && IgnoredValues[IgnoreIndex] == CurrentIndex) {
                    ++IgnoreIndex;
                    ++CurrentIndex;
                }
    ...
    template <class TTObjectsDataProvider, EFeatureType FeatureType, class TTFeatureHolderGenerator>
    static void GetPredictionsOnVaryingFeature(
        const TFullModel& model,
        const size_t featureNum,
        TTFeatureHolderGenerator featureHolderGenerator,
        ...
        CB_ENSURE(dataProviderPtr, "Only non-quantized datasets are supported");
    ...
    TBinarizedFeatureStatistics GetBinarizedFloatFeatureStatistics(
        ...
        if (auto* rawObjectsDataProvider =
            ...
            quantizedPtr =
                Quantize(options, rawObjectsDataProviderPtr, quantizedFeaturesInfo, &rand, &executor);
        } else if (
            auto* quantizedForCpuObjectsData =
        ...
        if (!isDatasetQuantized) {
            ...
            auto blockFunc = [&binNumsWriteIter, &poolBinsRemap](size_t /*blockStartIdx*/, auto block) {
              Transform(block.begin(), block.end(), binNumsWriteIter, [&poolBinsRemap](const auto binNum) {
                return poolBinsRemap[binNum];
              });
